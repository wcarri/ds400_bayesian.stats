...
```{r}
#ggplot(data = bikes, aes(x = , y = , shape = , size =, color =))+
#  geom_?
```

```{r, warning = FALSE}
ggplot(data = bikes, aes(x = season, fill = as.factir(rides)))+
...
```{r, message = FALSE}
ggplot(data = bikes, aes(x = windspeed, y = rides, color = weather_cat)) +
  geom_point() +
  geom_smooth(method = "lm") +
  stat_cor()
```
